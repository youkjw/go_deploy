...
type EventBiz interface {
	...
	CreateProject(action EventAction, id, name string, user web.User)
...
func (b *eventBiz) CreateProject(action EventAction, id, name string, user web.User) {
	args := data.Map{"id": id, "name": name}
	b.create(EventTypeProject, action, args, user)
}
